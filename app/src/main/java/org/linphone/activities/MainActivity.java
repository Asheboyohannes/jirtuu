...
public abstract class MainActivity extends LinphoneGenericActivity
    ...
    List<LinphoneContact> listc;
    JSONArray jsonArray;
    ArrayList<String> JirtuuContacts = new ArrayList<>();
    JSONObject contactscheck;

    public void InitializeJirtuNumbers() {
        if (LinphoneContext.isReady()) {
            ...
            handler.postDelayed(
                    new Runnable() {
                        ...
                        public void run() {
                            ...
                            listc = ContactsManager.getInstance().getContacts();
                            //                            Toast.makeText(MainActivity.this, "" +
                            // listc.size(), Toast.LENGTH_SHORT)
                            //                                    .show();
                            ...
                            if (listc.size() != 0) {
                                try {
                                    ...
                                    for (int i = 0; i < listc.size(); i++) {
                                        String no =
                                                listc.get(i)
                                                        .getNumbersOrAddresses()
                                                        .toString()
                                                        .replace("[tel:", "")
                                                        .replace("]", "");
                                        jsonArray.put(no);
                                    }
                                    contactscheck = new JSONObject();
                                    contactscheck.put("contacts", jsonArray);

                                    new ATask4CheckContacts().execute();
    ...
    private class ATask4CheckContacts extends AsyncTask<String, Void, String> {
        String Token = "";
        private final String MY_PREFS_NAME = "jirtuu";
        ...
        protected String doInBackground(String... params) {
            Response = api.Check_Contacts(contactscheck, Token);
            return Response;
        }
        ...
        protected void onPostExecute(String result) {
            if (result == null) {
                ...
                try {
                    JirtuuContacts.clear();
                    JSONObject obj = new JSONObject(result);
                    String con = obj.getString("contacts");
                    JSONArray jary = new JSONArray(con);
                    for (int y = 0; y < jary.length(); y++) {
                        JirtuuContacts.add(jary.getString(y));
                    }

                    Set<String> set = new HashSet<String>();
                    set.addAll(JirtuuContacts);
                    editor.putStringSet("jirtucontacts", set);
                    editor.commit();

                    //                Toast.makeText(MainActivity.this, "" + JirtuuContacts.size(),
                    // Toast.LENGTH_SHORT)
                    //                        .show();
